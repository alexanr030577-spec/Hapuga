...
class HuntService : Service() {
    companion object {
        ...
        const val EXTRA_MARKER_COUNT="markerCount"; const val EXTRA_RUNNING="running"; const val CHANNEL="hunt"
    }
    private var projection:MediaProjection?=null; private var reader:ImageReader?=null
    private var previous=emptyList<Pair<Int,Int>>(); private var lastDiagnostic=0L
    ...
    override fun onCreate(){super.onCreate();val f=IntentFilter(ACTION_QUERY_STATE);if(Build.VERSION.SDK_INT>=33)registerReceiver(queryReceiver,f,RECEIVER_NOT_EXPORTED)else @Suppress("DEPRECATION") registerReceiver(queryReceiver,f)}
    ...
    private fun stopHunt(){reader?.setOnImageAvailableListener(null,null);reader?.close();reader=null;projection?.stop();projection=null;previous=emptyList();stopForeground(STOP_FOREGROUND_REMOVE);broadcastState();stopSelf()}
    ...
    private fun purple(c:Int):Boolean{val r=android.graphics.Color.red(c);val g=android.graphics.Color.green(c);val b=android.graphics.Color.blue(c);return r in 145..235&&b in 180..255&&g<125&&b>g+65&&r>g+55}
    private fun light(c:Int):Boolean{val r=android.graphics.Color.red(c);val g=android.graphics.Color.green(c);val b=android.graphics.Color.blue(c);return r>205&&g>205&&b>205}
    private fun scan(bm:Bitmap,w:Int,h:Int){val cand=mutableListOf<Pair<Int,Int>>();for(y in 25 until h-25 step 9)for(x in 25 until w-25 step 9){if(!inZone(x.toDouble()/w,y.toDouble()/h))continue;var pn=0;var ln=0;for(dy in -21..21 step 7)for(dx in -21..21 step 7){val c=bm.getPixel(x+dx,y+dy);if(purple(c))pn++;if(abs(dx)<=14&&abs(dy)<=14&&light(c))ln++};if(pn>=16&&ln>=1)cand+=x to y};val clusters=mutableListOf<Pair<Int,Int>>();for(p in cand)if(clusters.none{abs(it.first-p.first)<70&&abs(it.second-p.second)<70})clusters+=p;val now=SystemClock.elapsedRealtime();if(now-lastDiagnostic>1000){sendBroadcast(Intent(ACTION_DIAGNOSTIC).setPackage(packageName).putExtra(EXTRA_MARKER_COUNT,clusters.size));lastDiagnostic=now};val isNew=previous.isNotEmpty()&&clusters.any{p->previous.none{q->abs(q.first-p.first)<78&&abs(q.second-p.second)<78}};if(isNew)sendBroadcast(Intent(ACTION_NEW_ORDER).setPackage(packageName));previous=clusters}
    ...
    override fun onDestroy(){try{unregisterReceiver(queryReceiver)}catch(_:Exception){};reader?.close();projection?.stop();handlerThread.quitSafely();super.onDestroy()}
}
